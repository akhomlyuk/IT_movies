...
> A catalog of films and series about computers, technology, artificial intelligence, etc
...
- `robots.txt`, `sitemap.xml` — SEO (regenerated by `tools/verify.py` from the single `SITE_BASE` when stale, `--no-write` for check-only; indexes home page + all film pages)
