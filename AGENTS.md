...
- `tools/` — local Python helper scripts (tracked in git):
  ...
  - `verify.py` — integrity checker (source of truth; verifies generated pages are fresh, theme/JSON-LD/noscript/metrika blocks are in sync, JS↔Python slug and related parity, regenerates `sitemap.xml` and `robots.txt` only when stale, `--no-write` = check-only, requires `.nojekyll`)
  - `gen_pages.py` — generates `films/*` pages from `js/data.js` + `js/i18n.js` (each embeds `window.FILM_PAGE` and the Yandex.Metrika snippet) and injects the theme, JSON-LD, no-JS and Metrika blocks into `index.html` / `404.html` via markers
...
- `films/*` pages are generated, NEVER edited by hand: change the data (or `tools/gen_pages.py`) and re-run it.
- The theme head-script, the Metrika snippet in `index.html`/`404.html` and the no-JS catalog block in `index.html` are generated by `tools/gen_pages.py` from single source (`gen_pages.THEME_SCRIPT`, `gen_pages.METRIKA_SCRIPT`, `gen_pages.index_noscript`) — never edit them by hand, re-run the generator instead.
