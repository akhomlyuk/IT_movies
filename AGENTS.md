...
- `index.html` — page markup, `<template id="tpl-catalog">` inline component template, Yandex.Metrika snippet. Three blocks are GENERATED by `gen_pages.py` (edit via markers, never by hand): the theme head-script (`<!-- begin:theme-script -->` … `<!-- end:theme-script -->`), the static JSON-LD (`<!-- begin:index-ld -->` … `<!-- end:index-ld -->`) and the no-JS catalog list (`<!-- begin:catalog-noscript -->` … `<!-- end:catalog-noscript -->`)
...
- `tools/` — local Python helper scripts (tracked in git):
  ...
  - `verify.py` — integrity checker (source of truth; verifies generated pages are fresh, theme/JSON-LD/noscript blocks are in sync, JS↔Python slug parity, regenerates `sitemap.xml` and `robots.txt` only when stale, `--no-write` = check-only, requires `.nojekyll`)
  - `gen_pages.py` — generates `films/*` pages from `js/data.js` + `js/i18n.js` (each embeds `window.FILM_PAGE`) and injects the theme, JSON-LD and no-JS blocks into `index.html` / `404.html` via markers
...
- The theme head-script in `index.html`/`404.html` and the no-JS catalog block in `index.html` are generated by `tools/gen_pages.py` from single source (`gen_pages.THEME_SCRIPT`, `gen_pages.index_noscript`) — never edit them by hand, re-run the generator instead.
